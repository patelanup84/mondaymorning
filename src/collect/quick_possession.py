...
class QPCollector(BaseCollector):
    ...
    def _initialize_qp_table(self) -> None:
        ...
        success = self.state_manager.create_table("urls", self.QP_SCHEMA, self.QP_INDEXES)
        if not success:
            raise Exception("Failed to initialize QP URLs table")
    
    def _has_fresh_data(self) -> bool:
        """Check if any URLs exist within the freshness window."""
        ...
        has_fresh = count > 0
        self.logger.info(f"Fresh data check: {count} URLs within {DATA_FRESHNESS_HOURS}h window (fresh: {has_fresh})")
        return has_fresh
    ...
    def _get_existing_competitors(self) -> Set[str]:
        ...
        results = self.state_manager.query(
            "SELECT DISTINCT competitor_id FROM urls WHERE discovered_at > ?",
            [cutoff_time.isoformat()]
        )
    ...
    def _add_discovered_urls(self, urls_data: List[Dict[str, Any]]) -> int:
        ...
        new_count = self.state_manager.insert_or_ignore("urls", urls_data)
        self.logger.info(f"Added {new_count} new URLs to cache")
        return new_count
    ...
    def _get_pending_urls_for_competitor(self, competitor_id: str, limit: int) -> List[Dict[str, Any]]:
        ...
        self.logger.info(f"Found {len(results)} pending URLs for {competitor_id} (limit: {limit})")
        return results
    ...
    def _update_extraction_result(self, url: str, extraction_result: Dict[str, Any]) -> bool:
        ...
        success = self.state_manager.update_record("urls", updates, "url = ?", [url])
    ...
    def _get_extraction_stats(self) -> Dict[str, int]:
        ...
        stats = self.state_manager.get_stats("urls", "extraction_status")
    ...
    async def _collect_raw(self, config: QPCollectorConfig) -> bool:
        ...
        try:
            ...
            self.logger.info(f"URL discovery complete: {total_urls} total URLs, {pending_count} pending extraction")
    ...
    async def _transform(self, config: QPCollectorConfig) -> bool:
        ...
        try:
            ...
            if not all_pending_urls:
                self.logger.info("No pending URLs to extract")
                self.processed_data = []
                return True
            
            self.logger.info(f"Starting extraction for {len(all_pending_urls)} pending URLs")
            ...
            for i, url_data in enumerate(all_pending_urls, 1):
                ...
                try:
                    self.logger.info(f"Extracting URL {i}/{len(all_pending_urls)}: {url}")
